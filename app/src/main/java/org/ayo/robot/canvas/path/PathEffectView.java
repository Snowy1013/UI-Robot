...
public class PathEffectView extends ShapeView {
    ...
    public void setPathEffect(List<String> pathEffects, boolean isCompose, boolean isSum){
        if(pathEffects == null || pathEffects.size() == 0){
            ...
            if(isCompose && isSum){
                Toaster.toastShort("compose和sum这俩就能学一个");
            }else if(isCompose || isSum){
                if(pathEffects.size() != 2){
                    Toaster.toastShort("compose或者sum要求必须有两个输入");
                }else{
                    ...
                    if(isCompose){
                        ...
                        Toaster.toastShort("按选择的先后顺序compose--先选是out，后选是in");
                        ...
                        Toaster.toastShort("按选择的先后顺序sum--先选是first，后选是second");
                        getPaint().setPathEffect(composePathEffect);
                        invalidate();
